...
function generateRecommendations(fonts, usedFonts) {
  ...
  const unusedFonts = fonts.filter(font => {
    ...
    return !usedFonts.some(used => fontName.includes(used.toLowerCase()) || used.toLowerCase().includes(fontName));
  });
  ...
  fonts.forEach(font => {
    const baseName = font.name.replace(/[-_](bold|regular|light|medium|semibold|black|thin|extralight|heavy).*\.(ttf|otf|woff|woff2)$/i, '');
  ...
  const multipleWeights = Array.from(fontFamilies.entries()).filter(([_, variants]) => variants.length > 2);
...
function main() {
  ...
  try {
    ...
    const fonts = [
      ...findFontFiles(ASSETS_DIR),
      ...findFontFiles(ANDROID_FONTS_DIR),
    ];
