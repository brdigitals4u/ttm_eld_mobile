...
const SOURCE_MAP = path.join(__dirname, '../android/app/build/intermediates/sourcemaps/react/release/index.android.bundle.packager.map');
...
function generateRecommendations(data) {
  ...
  const foundLarge = data.modules.filter(m => 
    largeLibraries.some(lib => m.includes(lib))
  );
...
function runSourceMapExplorer() {
  ...
  try {
    const output = execSync(
      `npx source-map-explorer "${BUNDLE_OUTPUT}" "${SOURCE_MAP}" --json`,
