...
function identifyLargeDependencies(sizes) {
  ...
  const large = sizes.filter(pkg => 
    pkg.size > largeThreshold || knownLarge.some(name => pkg.name.includes(name))
  );
  ...
  report.recommendations.forEach((rec, i) => {
    ...
    if (rec.items && rec.items.length > 0) {
      rec.items.slice(0, 5).forEach(item => {
        const itemStr = typeof item === 'string' ? item : `${item.name} (${item.sizeFormatted || ''})`;
        summary += `   - ${itemStr}\n`;
      });
