...
function findLatestBuild() {
  ...
  if (fs.existsSync(aabDir)) {
    const aabs = fs.readdirSync(aabDir)
...
function generateReport(history, currentBuild) {
  ...
  if (history.builds.length >= 2) {
    ...
    report.trend = recent.map((build, i) => {
      if (i === 0) return null;
...
function printReport(report) {
  ...
  if (report.comparison) {
    ...
    console.log(`   ${icon} ${sign}${report.comparison.sizeDiffFormatted} (${sign}${Math.abs(report.comparison.percentDiff)}%)`);
  ...
  if (report.trend.length > 0) {
    ...
    report.trend.forEach((change, i) => {
      ...
      console.log(`   ${icon} ${sign}${change.change} (${sign}${Math.abs(parseFloat(change.percent))}%)`);
    });
  }
