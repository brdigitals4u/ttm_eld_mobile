...
const BUNDLE_OUTPUT = path.join(__dirname, '../android/app/build/generated/assets/createBundleReleaseJsAndAssets/index.android.bundle');
...
function analyzeImports(filePath) {
  ...
  const importRegex = /import\s+(?:(?:\*\s+as\s+\w+)|(?:\{[^}]*\})|(?:\w+)|(?:\w+,\s*\{[^}]*\}))\s+from\s+['"]([^'"]+)['"]/g;
  let match;
...
function identifyHeavyFeatures(files) {
  const heavyPatterns = [
    ...
    { pattern: /chart|Chart|victory|Victory/, name: 'Charts', suggestion: 'Consider lazy loading chart libraries' },
    { pattern: /realm|Realm/, name: 'Realm Database', suggestion: 'Ensure Realm is tree-shakeable' },
    ...
    if (rec.items && rec.items.length > 0) {
      rec.items.slice(0, 5).forEach(item => {
        const itemStr = typeof item === 'string' ? item : 
          item.file ? `${item.file}${item.module ? ` -> ${item.module}` : ''}${item.size ? ` (${item.size})` : ''}` :
