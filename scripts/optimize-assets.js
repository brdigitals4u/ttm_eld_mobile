...
try {
  ...
  console.warn('⚠️  imagemin modules not found. Install with: npm install -D imagemin imagemin-webp imagemin-pngquant');
}
    ...
    try {
      ...
      if (optimizedPath) {
        ...
        if (shouldConvert && optimizedPath.endsWith('.webp')) {
          stats.converted++;
          console.log(`✅ Converted: ${relativePath} → ${path.basename(optimizedPath)} (${formatSize(originalSize)} → ${formatSize(optimizedSize)}, saved ${formatSize(saved)})`);
        } else {
          stats.optimized++;
          console.log(`✅ Optimized: ${relativePath} (${formatSize(originalSize)} → ${formatSize(optimizedSize)}, saved ${formatSize(saved)})`);
        }
      } else {
        stats.skipped++;
