...
export default function AssetsScreen() {
  ...
  const handleSubmit = async () => {
    if (!formData.number) {
      Alert.alert('Missing Information', 'Please enter the asset number.');
      return;
    }
    ...
    <Card style={styles.assetCard}>
      <View style={styles.assetHeader}>
        <View style={styles.assetInfo}>
          ...
          <View style={styles.assetDetails}>
            ...
            <Text style={[styles.assetType, { color: colors.inactive }]}>
              {item.type.charAt(0).toUpperCase() + item.type.slice(1)}
            </Text>
            {item.make && item.model && (
              <Text style={[styles.assetMakeModel, { color: colors.inactive }]}>
        ...
        <View style={[
          styles.statusBadge,
          { backgroundColor: item.isActive ? colors.success : colors.inactive }
        ]}>
      ...
      {(item.vin || item.licensePlate) && (
        <View style={styles.assetMeta}>
          {item.vin && (
            <Text style={[styles.assetMetaText, { color: colors.inactive }]}>
              VIN: {item.vin}
            </Text>
          )}
          {item.licensePlate && (
      ...
      <View style={styles.assetActions}>
        <View style={styles.documentsInfo}>
          <FileText size={16} color={colors.inactive} />
          <Text style={[styles.documentsCount, { color: colors.inactive }]}>
              ...
              {[
                ...
                <Pressable
                  ...
                  style={[
                    styles.typeButton,
                    {
                      backgroundColor: formData.type === type.key ? colors.primary : 'transparent',
                ...
                >
                  <Text
                    style={[
                      ...
                      {
                        color: formData.type === type.key ? '#fff' : colors.primary,
                      },
                    ]}
                  >
                    {type.label}
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {assets.length === 0 ? (
        <Card style={styles.emptyContainer}>
          <Truck size={48} color={colors.inactive} />
