...
import { useDashboardData, useCertifyLogs } from '@/hooks/api';

export default function DashboardScreen() {
  const { colors, isDark } = useTheme();
  const { user, vehicleInfo } = useAuth();
  const { connectedDevice } = useEld();
  ...
  const handleCertifyLogs = async () => {
    try {
      await certifyLogs();
      Alert.alert('Success', 'Logs have been certified successfully');
      setShowUncertifiedModal(false);
    } catch (error) {
      Alert.alert('Error', 'Failed to certify logs. Please try again.');
    }
  };
  ...
  return (
    ...
    >
      ...
      <View style={[styles.connectionStatus, { 
        backgroundColor: dashboardData?.connectionStatus === 'connected' ? '#E8F5E8' : '#FFF3E0' 
      }]}>
        <Link size={16} color={dashboardData?.connectionStatus === 'connected' ? '#4CAF50' : '#FF9500'} />
        <Text style={[styles.connectionText, { 
          color: dashboardData?.connectionStatus === 'connected' ? '#4CAF50' : '#FF9500' 
        }]}>
      ...
      <View style={styles.cycleInfo}>
        <Text style={[styles.cycleText, { color: colors.inactive }]}>
      ...
      <View style={styles.mainTimerContainer}>
        <View style={styles.timerCircle}>
          ...
          <Text style={[styles.mainTimeLabel, { color: colors.inactive }]}>
            Stop In
          </Text>
        </View>
      </View>
      ...
      <View style={styles.timeBarsContainer}>
        <View style={styles.timeBarRow}>
          <View style={styles.timeBarItem}>
            ...
            <Text style={[styles.timeBarLabel, { color: colors.inactive }]}>
              Drive Left
            </Text>
        ...
        <View style={styles.cycleTimeContainer}>
          <Text style={[styles.cycleLabel, { color: colors.inactive }]}>
            Cycle Left
          </Text>
            ...
            <View style={styles.modalButtons}>
              ...
              >
                <Text style={styles.modalButtonText}>
                  {certifyLoading ? 'CERTIFYING...' : 'REVIEW AND CERTIFY'}
                </Text>
              </TouchableOpacity>
      ...
      <View style={styles.vehicleInfo}>
        ...
        <Text style={[styles.vehicleInfoValue, { color: colors.text }]}>
          {vehicleInfo?.vehicleNumber || 'No vehicle assigned'}
        </Text>
      </View>
    </ScrollView>
  );
