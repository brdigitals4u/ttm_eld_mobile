...
export default function InspectionScreen() {
    ...
    if (status === 'fail') {
      Alert.prompt(
        ...
        [
          ...
          { 
            text: 'Save', 
            onPress: (notes) => updateInspectionItem(item.id, status, notes || undefined)
          },
        ],
        'plain-text'
      );
  ...
  const handleCompleteInspection = () => {
    ...
    if (pendingRequired.length > 0) {
      Alert.alert(
        ...
        `Please complete all required items (${pendingRequired.length} remaining).`,
        [{ text: 'OK' }]
      );
      return;
    }
  ...
  const handleSubmitSignature = async () => {
    if (!signature.trim()) {
      Alert.alert('Missing Signature', 'Please enter your signature to complete the inspection.');
      return;
    }
  ...
  const getStatusIcon = (status: 'pass' | 'fail' | 'na' | 'pending') => {
    switch (status) {
      ...
      case 'na':
        return <Text style={{ color: colors.inactive, fontSize: 16, fontWeight: '600' as const }}>N/A</Text>;
      default:
  ...
  const renderInspectionItem = ({ item }: { item: InspectionItem }) => (
    <Card style={styles.inspectionItem}>
      <View style={styles.itemHeader}>
        <View style={styles.itemInfo}>
          <Text style={[styles.itemCategory, { color: colors.inactive }]}>
            {item.category}
          </Text>
          <Text style={[styles.itemName, { color: colors.text }]}>
            {item.item}
            {item.isRequired && <Text style={{ color: colors.danger }}> *</Text>}
          </Text>
          {item.notes && (
            <Text style={[styles.itemNotes, { color: colors.inactive }]}>
              Notes: {item.notes}
            </Text>
          )}
        </View>
      ...
      <View style={styles.itemActions}>
        <Pressable
          ...
          style={[
            styles.statusButton,
            {
              backgroundColor: item.status === 'pass' ? colors.success : 'transparent',
        ...
        >
          <Text
            style={[
              ...
              {
                color: item.status === 'pass' ? '#fff' : colors.success,
              },
            ]}
          >
            Pass
        ...
        <Pressable
          ...
          style={[
            styles.statusButton,
            {
              backgroundColor: item.status === 'fail' ? colors.danger : 'transparent',
        ...
        >
          <Text
            style={[
              styles.statusButtonText,
              {
                color: item.status === 'fail' ? '#fff' : colors.danger,
              },
            ]}
          >
            Fail
        ...
        <Pressable
          ...
          style={[
            styles.statusButton,
            {
              backgroundColor: item.status === 'na' ? colors.inactive : 'transparent',
        ...
        >
          <Text
            style={[
              styles.statusButtonText,
              {
                color: item.status === 'na' ? '#fff' : colors.inactive,
              },
            ]}
          >
            N/A
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={styles.signatureForm}>
          <Card>
            ...
            <Text style={[styles.signatureDescription, { color: colors.inactive }]}>
      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <View style={styles.startForm}>
          <Card>
            ...
            <Text style={[styles.startDescription, { color: colors.inactive }]}>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <Card style={styles.progressCard}>
        ...
        <View style={[styles.progressBar, { backgroundColor: isDark ? colors.card : '#E5E7EB' }]}>
          <View
            style={[
              styles.progressFill,
              {
