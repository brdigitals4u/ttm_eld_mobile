...
export const driverProfileApi = {
  ...
  async getChangeRequests(): Promise<ChangeRequestsResponse> {
    console.log('📋 Fetching change requests from:', API_ENDPOINTS.ORGANIZATION.DRIVER_PROFILE_CHANGE_REQUESTS)
    ...
    if (response.data) {
      ...
      if (Array.isArray(response.data)) {
        console.log('📋 Response data is array, wrapping in requests field. Count:', response.data.length)
        result = { 
      ...
      else if (response.data.count !== undefined || (typeof response.data === 'object' && !Array.isArray(response.data))) {
  ...
  return useMutation({
    ...
    onSuccess: (response) => {
      ...
      if (authStore.driverProfile && response.driver) {
        ...
        if (response.updated_fields.includes('home_terminal_address') && response.driver.home_terminal_address !== undefined) {
        ...
        if (response.updated_fields.includes('phone') && authStore.user && response.driver.phone !== undefined) {
          useAuthStore.setState({ 
            user: {
              ...authStore.user,
