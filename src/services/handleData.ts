  ...
  interface OBDDataItem {
    ...
    const pidMap: { [key: string]: string } = {
      ...
      '0558':
        'Aftertreatment 2 Diesel Particulate Filter Intake Gas Temperature',
      ...
      '055c':
        'Aftertreatment 2 Diesel Particulate Filter Differential Pressure',
...
export const handleData = (data: any) => {
  ...
  if (data.dataFlowList && Array.isArray(data.dataFlowList)) {
    console.log(
      '📊 Processing dataFlowList with',
      data.dataFlowList.length,
      'items',
    );
    ...
    data.dataFlowList.forEach((item: any, index: number) => {
      ...
      if (pid && value !== undefined && !isNaN(value)) {
        ...
        console.log(`❌ PID: ${pid}, Value: ${value}, isNaN: ${value !== undefined ? isNaN(value) : 'undefined'}`);
      }
    });
  }
