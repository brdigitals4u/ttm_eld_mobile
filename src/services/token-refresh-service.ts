...
class TokenRefreshService {
  ...
  async setTokenExpiry(expiresIn: number): Promise<void> {
    try {
      ...
      console.log('⏰ TokenRefreshService: Token expiry stored:', new Date(expiryTime).toISOString())
    } catch (error) {
  ...
  async refreshToken(): Promise<boolean> {
    ...
    const refreshPromise = (async (): Promise<boolean> => {
      try {
        ...
        console.log('⏰ TokenRefreshService: New expiry:', new Date(Date.now() + expiresIn * 1000).toISOString())
