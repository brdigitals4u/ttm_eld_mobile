  ...
  async smartFetch(options: SmartFetchOptions = {}): Promise<EldRecord[]> {
    ...
    try {
      ...
      if (has5minData) {
        ...
        console.log(`✅ Smart Fetch: Found ${records5min.length} records in 5min, escalating to 20min...`)
        ...
        if (has20minData) {
          ...
          console.log(`✅ Smart Fetch: Found ${records20min.length} records in 20min, escalating to 4hr...`)
          ...
          if (has4hrData) {
            ...
            console.log(`✅ Smart Fetch: Found ${records4hr.length} records in 4hr, escalating to 24hr...`)
      ...
      const uniqueRecords = Array.from(
        new Map(allRecords.map(record => [record.id || `${record.eventTime}-${record.eventType}`, record])).values()
      )
