...
import { JMBluetoothEvents } from "../types/JMBluetooth"
...
class BluetoothLogger {
  ...
  getRecentLogs(
    count: number = 50,
  ): Array<{ timestamp: string; level: string; message: string; data?: any }> {
...
class JMBluetoothService {
  ...
  async checkPasswordEnable(): Promise<boolean> {
    try {
      ...
      logger.error("Failed to check password enable:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async startReportEldData(): Promise<boolean> {
    try {
      ...
      logger.error("Failed to start ELD data reporting:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async stopReportEldData(): Promise<boolean> {
    try {
      ...
      logger.error("Failed to stop ELD data reporting:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async startReportObdData(): Promise<boolean> {
    try {
      ...
      logger.error("Failed to start OBD data reporting:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async stopReportObdData(): Promise<boolean> {
    try {
      ...
      logger.error("Failed to stop OBD data reporting:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async stopReportHistoryData(): Promise<boolean> {
    try {
      ...
      logger.error("Failed to stop history data reporting:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async queryDataItemConfig(): Promise<boolean> {
    try {
      ...
      logger.error("Failed to query data item config:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async setDataItemConfig(config: string): Promise<boolean> {
    try {
      ...
      logger.error("Failed to set data item config:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async setDataItemConfigBatch(pids: number[]): Promise<boolean> {
    try {
      ...
      logger.error("Failed to set data item config batch:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async acknowledgeCustomCommand(): Promise<boolean> {
    try {
      ...
      logger.error("Failed to acknowledge custom command reply:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async saveDriverAuthInfo(info: string): Promise<boolean> {
    try {
      ...
      logger.error("Failed to save driver auth info:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async readDriverAuthInfo(): Promise<boolean> {
    try {
      ...
      logger.error("Failed to read driver auth info:", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async replyDpfRegenerationUploadState(): Promise<boolean> {
    try {
      ...
      logger.error("Failed to acknowledge DPF upload state", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
  ...
  async checkDpfRegenerationState(): Promise<boolean> {
    try {
      ...
      logger.error("Failed to check DPF regeneration state", {
        error: error?.message,
        stack: error?.stack,
      })
      throw error
    ...
    const wrappedListener = (data: any) => {
      ...
      if (event === "onConnected") {
        logger.info("Device connected successfully:", {
          timestamp: new Date().toISOString(),
          attemptNumber: this.connectionAttempts,
          timeSinceLastDisconnect: this.lastDisconnectionTime
            ? Date.now() - this.lastDisconnectionTime
            : null,
        })
      }
      ...
      if (event === "onObdErrorDataReceived") {
        ...
        if (ecuList.length === 0) {
          logger.warn(
            "⚠️ JMBluetoothService: onObdErrorDataReceived event has empty ecuList array",
            {
              ecuCount: data?.ecuCount,
        ...
        ecuList.forEach((ecu: any, index: number) => {
          ...
          logger.debug(
            `📋 JMBluetoothService: ECU ${index} - ID: ${ecu?.ecuId || ecu?.ecuIdHex || "unknown"}, Codes: ${JSON.stringify(codes)}`,
          )
        ...
        if (allCodes.length > 0) {
          ...
          if (allCodes.includes("P0195")) {
            logger.warn(
              '🔍 JMBluetoothService: P0195 DETECTED - Engine Oil Temperature Sensor "A" Circuit Malfunction',
              {
                code: "P0195",
  ...
  getConnectionStats() {
    return {
      ...
      currentConnectionDuration:
        this.lastConnectionTime && !this.lastDisconnectionTime
    ...
    try {
      ...
      const deviceId =
        typeof response === "string"
          ? response
          : (response?.deviceId ?? response?.device_id ?? response?.address ?? response?.macAddress)
