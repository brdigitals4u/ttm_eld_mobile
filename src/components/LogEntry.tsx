...
function LogEntry({ log, onCertify }: LogEntryProps) {
  ...
  const getStatusColor = () => {
    ...
    switch (status) {
      case "driving":
        return colors.warning // Orange for driving
      case "onDuty":
        return colors.warning // Amber for on duty
      case "offDuty":
        return colors.tint // Blue for off duty
      case "sleeping":
      case "sleeperBerth":
        return colors.palette.primary600 // Indigo for sleeping
      case "personalConveyance":
        return colors.success // Green for personal conveyance
      case "yardMove":
        return colors.warning // Warning color
      default:
    ...
    >
      ...
      <View style={styles.compactRow}>
        <View style={styles.leftSection}>
          ...
          <Text style={[styles.time, { color: colors.textDim }]}>{formatTime(getTimestamp())}</Text>
        </View>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.background }]}>
            <Text style={[styles.modalTitle, { color: colors.text }]}>Certify Log Entry</Text>
