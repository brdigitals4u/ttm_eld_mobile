...
export default function HOSChart({ data = [], dayStartIso, header = {}, onSegmentPress, onRenderPlan }: HOSChartProps) {
  ...
  const segments = useMemo<SegmentRender[]>(() => {
    const mapped: SegmentRender[] = data
      .map((s) => {
        ...
        const endMinutes = Math.min(24 * 60, Math.max(startMinutes, minutesBetween(dayStart, endClipped)));
    ...
    const pushSegment = (segment: SegmentRender) => {
      ...
      if (last && last.statusKey === segment.statusKey && segment.startMinutes - last.endMinutes <= 1) {
  ...
  const totalMinutes = useMemo(() => segments.reduce((sum, seg) => sum + seg.durationMin, 0), [segments]);
  ...
  const renderPlan = useMemo(() => {
    ...
    const plan = {
      ...
      grid: {
        quarterTicks: quarterHourTicks.map((tick) => ({ minutes: tick.minutes, x: tick.x, isHour: tick.isHour, isHalf: tick.isHalf })),
      },
      validation: {
    ...
    return plan;
  }, [renderData, quarterHourTicks, drawingWidth, minPixelWidth, totalMinutes, dayStartIso, screenWidth]);
    ...
    <View style={styles.container}>
      <View style={[styles.chartContainer, { width: chartWidth }]}>
        ...
        <Svg width={chartWidth} height={TOTAL_HEIGHT}>
          ...
          <SvgLine x1={LEFT_MARGIN} y1={TOP_MARGIN + CHART_HEIGHT} x2={LEFT_MARGIN + drawingWidth} y2={TOP_MARGIN + CHART_HEIGHT} stroke="#9CA3AF" strokeWidth={2} />
          ...
          {quarterHourTicks.map(({ minutes, x, isHour, isHalf }) => (
            <React.Fragment key={minutes}>
              <SvgLine
                ...
                y1={TOP_MARGIN}
                x2={x}
                y2={isHour ? TOP_MARGIN + CHART_HEIGHT : isHalf ? TOP_MARGIN + CHART_HEIGHT * 0.85 : TOP_MARGIN + CHART_HEIGHT * 0.7}
                stroke={isHour ? "#9CA3AF" : isHalf ? "#CBD5F5" : "#E5E7EB"}
          ...
          {Object.entries(renderData.rows).map(([key, row]) => (
            <React.Fragment key={key}>
              {row.path.length > 0 && (
                <Path d={row.path} stroke="#1F2937" strokeWidth={3} strokeLinecap="round" fill="none" />
              )}
              {row.markers.map((marker, markerIndex) => (
                <Rect key={`marker-${key}-${markerIndex}`} x={marker.x - 3} y={marker.y - 3} width={6} height={6} fill="#1F2937" rx={1} />
              ))}
              ...
              {row.tinyDots.map((dot, dotIndex) => (
                <Circle key={`dot-${key}-${dotIndex}`} cx={dot.cx} cy={dot.cy} r={3} fill="#1F2937" />
              ))}
            </React.Fragment>
          ))}
        </Svg>
