...
export default function HOSChart({ logs, date, driverName, vehicleNumber }: HOSChartProps) {
  ...
  const renderStepLineChart = () => {
    ...
    for (const hour of hourValues) {
      ...
      if (hour % 2 === 0) {
        hourMarkers.push(<View key={`marker-${hour}`} style={[styles.hourMarker, { left: x }]} />)
      }
    ...
    for (let level = 1; level <= 4; level++) {
      ...
      statusMarkers.push(
        <View key={`status-marker-${level}`} style={[styles.statusMarker, { top: y }]} />,
      )
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          ...
          <Text style={[styles.headerText, { color: colors.text }]}>{formattedDate}</Text>
        </View>
        <View style={styles.headerRight}>
          <View style={styles.driverInfo}>
            <User size={16} color={colors.textDim} />
            <Text style={[styles.driverText, { color: colors.textDim }]}>{driverName}</Text>
          </View>
          <View style={styles.vehicleInfo}>
            <Truck size={16} color={colors.textDim} />
            <Text style={[styles.vehicleText, { color: colors.textDim }]}>{vehicleNumber}</Text>
          </View>
        </View>
      </View>

      {/* Chart */}
      <View style={styles.chartContainer}>{renderStepLineChart()}</View>
