...
interface AssetsContextType extends AssetsState {
  ...
  addDocument: (
    assetId: string,
    document: Omit<AssetDocument, "id" | "uploadedAt">,
  ) => Promise<void>
...
export const [AssetsProvider, useAssets] = createContextHook(() => {
  ...
  const addDocument = async (
    assetId: string,
    documentData: Omit<AssetDocument, "id" | "uploadedAt">,
  ) => {
    try {
      ...
      const updatedAssets = (state.assets || []).map((asset) =>
        asset.id === assetId ? { ...asset, documents: [...asset.documents, newDocument] } : asset,
      )
