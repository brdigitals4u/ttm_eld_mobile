...
export const ObdDataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [awsSyncStatus, setAwsSyncStatus] = useState<"idle" | "syncing" | "success" | "error">(
    "idle",
  )
  ...
  const connectionStateBeforeHistoryRef = useRef<{
    isConnected: boolean
    deviceAddress: string | null
  } | null>(null) // Store connection state before history fetch
  ...
  const completeHistoryFetch = useCallback(() => {
    ...
    console.log(
      `📊 OBD Data Context: Completing history fetch - ${recordCount} records collected`,
      {
    ...
    if (recordCount === 0) {
      ...
      console.warn(
        "  3. Device may only store data when certain conditions are met (e.g., engine on)",
      )
      ...
      console.warn(
        "  6. Data may be arriving but not being captured (check logs for data received events)",
      )
    } else {
    ...
    if (connectionStateBeforeHistoryRef.current) {
      console.log(
        "🔄 OBD Data Context: Restoring connection state after history fetch completion",
        {
    ...
    async ({ type, start, end }: EldHistoryRequest) => {
      ...
      const monitorConnection = async () => {
        try {
          ...
          if (!status.isConnected && connectionStateBeforeHistoryRef.current?.isConnected) {
            console.warn(
              "⚠️ OBD Data Context: Connection lost during history fetch, attempting reconnection...",
            )
            const deviceToReconnect =
              connectionStateBeforeHistoryRef.current.deviceAddress ||
            ...
            if (deviceToReconnect && status.isBluetoothEnabled && status.isBLESupported) {
              try {
                ...
                console.log(
                  "✅ OBD Data Context: Reconnection attempt initiated during history fetch",
                )
                ...
                console.error(
                  "❌ OBD Data Context: Failed to reconnect during history fetch",
                  reconnectError,
                )
              ...
              throw new Error(
                "Connection lost during history fetch and no device available for reconnection",
              )
            }
          }
        } catch (error) {
          console.error(
            "❌ OBD Data Context: Error monitoring connection during history fetch",
            error,
          )
        }
      }
      ...
      const historyFetchTimeout = setTimeout(() => {
        if (isFetchingHistoryRef.current) {
          const recordCount = historyRecordsRef.current.length
          console.log(
            `⏱️ OBD Data Context: History fetch extended timeout (60s) - collected ${recordCount} records`,
          )
      ...
      try {
        ...
        if (recordCount > 0) {
          ...
          console.log(
            `📊 OBD Data Context: After 13s total wait - ${recordCount} records collected`,
          )

          if (recordCount > 0) {
            console.log(
              "📊 OBD Data Context: More data arriving! Step 3 - Additional wait (20 seconds)...",
            )
            await new Promise((resolve) => setTimeout(resolve, 20000))
            recordCount = historyRecordsRef.current.length
            console.log(
              `📊 OBD Data Context: After 33s total wait - ${recordCount} records collected`,
            )
          }
        } else {
          console.log(
            "⚠️ OBD Data Context: No data received after 3s, waiting longer (10 seconds)...",
          )
          await new Promise((resolve) => setTimeout(resolve, 10000))
          recordCount = historyRecordsRef.current.length
          console.log(
        ...
        console.log(
          "ℹ️ OBD Data Context: History fetch query completed, but flag remains active to capture delayed data",
          {
            isFetchingHistoryRef: isFetchingHistoryRef.current,
            isHistoryFetchingRef: isHistoryFetchingRef.current,
            recordCount: historyRecordsRef.current.length,
  ...
  useEffect(() => {
    ...
    const connectedListener = JMBluetoothService.addEventListener("onConnected", async () => {
        ...
        if (status.isBluetoothEnabled && status.isBLESupported && !status.isConnected) {
          ...
          if (savedDevice?.address) {
            ...
            try {
              ...
              console.warn(
                "⚠️ OBD Data Context: Reconnection failed, user may need to manually connect:",
                reconnectError,
              )
            }
          }
      ...
      if (prevState.match(/inactive|background/) && nextAppState === "active") {
        ...
        try {
          ...
          if (status.isConnected) {
            ...
            if (deviceToReconnect && status.isBluetoothEnabled && status.isBLESupported) {
              console.log(
                "🔄 OBD Data Context: Device disconnected on foreground, attempting reconnection...",
                {
  ...
  useEffect(() => {
    ...
    const obdEldDataListener = JMBluetoothService.addEventListener(
      ...
      (data: ObdEldData) => {
        ...
        console.log(
          "📊 OBD Data Context: Has dataFlowList?",
          !!data.dataFlowList,
          "Length:",
          data.dataFlowList?.length || 0,
        )
        console.log(
          "📊 OBD Data Context: driverProfile exists?",
          !!driverProfile,
          "driver_id:",
        ...
        const isHistoryEvent = isFetchingHistoryRef.current || isLiveEventFlag === 0
        ...
        if (isFetchingHistoryRef.current) {
          ...
          if (Math.random() < 0.01) {
            // Log 1% of non-history-fetch data
        ...
        const speedItem = displayData.find(
          (item) =>
        ...
        const vehicleVoltageItem = displayData.find((item) =>
        ...
        if (
          payloadWithDeviceId.latitude !== undefined &&
          payloadWithDeviceId.longitude !== undefined &&
          !isNaN(payloadWithDeviceId.latitude) &&
          !isNaN(payloadWithDeviceId.longitude)
        ) {
          setEldLocation({
            latitude: payloadWithDeviceId.latitude,
            longitude: payloadWithDeviceId.longitude,
          ...
          console.log(
            "📍 OBD Data Context: Stored ELD location:",
            payloadWithDeviceId.latitude,
            payloadWithDeviceId.longitude,
          )

          // Add location to queue for batch upload (new driver API)
          const odometerItem = displayData.find(
            (item) =>
        ...
        if (!driverProfile?.driver_id) {
          console.warn(
            "⚠️ OBD Data Context: Cannot add to buffers - missing driverProfile.driver_id",
          )
        ...
        console.log(
          "📦 OBD Data Context: Preparing payloads - vehicleId:",
          vehicleId,
          "timestamp:",
          timestamp,
        )
        ...
        if (vehicleId !== "UNKNOWN_VEHICLE" && timestamp > 0) {
          ...
          console.log(
            `📦 OBD Data Context: Added to buffers - Local: ${dataBufferRef.current.length}, AWS: ${awsBufferRef.current.length} items`,
          )
    ...
    const connectedListener = JMBluetoothService.addEventListener("onConnected", async () => {
      ...
      setTimeout(async () => {
        try {
          if (eldReportingStartedRef.current) {
            console.log(
              "ℹ️ OBD Data Context: ELD reporting already started during wait period, skipping",
            )
            return
          }
          ...
          if (status.isConnected) {
            ...
            try {
              ...
              console.log(
                "✅ OBD Data Context: ELD reporting started successfully from onConnected",
              )
            } catch (startError) {
              console.error(
                "❌ OBD Data Context: Failed to start ELD reporting from onConnected:",
                startError,
              )
        ...
        try {
          ...
          await eldSmartHistoryFetch.smartFetch({
            onProgress: (stage, hasData, recordCount) => {
              console.log(
                `📥 Smart history fetch: ${stage} - ${hasData ? `Found ${recordCount} records` : "No data"}`,
              )
            },
            onComplete: (records) => {
              console.log(
                `✅ OBD Data Context: Smart history fetch completed with ${records.length} records`,
              )
            },
            onError: (error, stage) => {
      ...
      async () => {
        ...
        setTimeout(async () => {
          try {
            if (eldReportingStartedRef.current) {
              console.log(
                "ℹ️ OBD Data Context: ELD reporting already started, skipping duplicate call",
              )
              return
            }
            ...
            if (status.isConnected) {
              console.log(
                "📊 OBD Data Context: Starting ELD reporting after authentication (fallback)...",
              )
              try {
                await JMBluetoothService.startReportEldData()
                eldReportingStartedRef.current = true
                console.log(
                  "✅ OBD Data Context: ELD reporting started successfully after authentication",
                )
              } catch (startError) {
                console.error(
                  "❌ OBD Data Context: Failed to start ELD reporting after authentication:",
                  startError,
                )
                // Reset flag on error so it can be retried
                eldReportingStartedRef.current = false
            ...
            console.log(
              "⚠️ OBD Data Context: Failed to start ELD reporting after authentication:",
              error,
            )
            // Reset flag on error so it can be retried
            eldReportingStartedRef.current = false
    ...
    const checkAndStartELDReporting = async () => {
      try {
        ...
        if (status.isConnected) {
          ...
          if (!isConnected) {
            console.log(
              "🔄 OBD Data Context: Updating connection state for ELD reporting (bypassing history fetch lock)",
            )
            setIsConnected(true)
            setActivityState("idling")
            if (status.currentDevice) {
            ...
            try {
              ...
              if (recheckStatus.isConnected && !eldReportingStartedRef.current) {
                ...
                try {
                  ...
                  eldReportingStartedRef.current = true
                  console.log(
                    "✅ OBD Data Context: ELD reporting started successfully from status check",
                  )
                } catch (startError) {
              ...
              console.log(
                "⚠️ OBD Data Context: Failed to start ELD reporting from status check:",
                error,
              )
              // Reset flag on error so it can be retried
              eldReportingStartedRef.current = false
        ...
        console.log(
          "⚠️ OBD Data Context: Failed to check connection status for ELD reporting:",
          error,
        )
      }
    }
          ...
          locationQueueService.setAutoDutyChangeHandler((changes) => {
            if (changes && changes.length > 0) {
              ...
              changes.forEach((change, index) => {
                console.log(
                  `🔄 OBD Data Context: Auto-duty change ${index + 1}/${changes.length}`,
                  {
                    seq: change.seq,
              ...
              if (latestChange.old_status === "driving" && latestChange.new_status === "on_duty") {
                console.log("🛑 OBD Data Context: Vehicle stopped - Auto-returning to On Duty", {
                  ...
                  idleTime: latestChange.reason?.toLowerCase().includes("idle")
                    ? "detected"
                    : "unknown",
                })
              } else if (
                latestChange.old_status === "on_duty" &&
                latestChange.new_status === "driving"
              ) {
    ...
    const obdEldStartListener = JMBluetoothService.addEventListener("onObdEldStart", () => {
      console.log("📊 OBD Data Context: OBD ELD started")
      setIsConnected(true)
    })

    const obdEldFinishListener = JMBluetoothService.addEventListener("onObdEldFinish", () => {
      console.log("📊 OBD Data Context: OBD ELD finished event received")
      if (isFetchingHistoryRef.current) {
        const recordCount = historyRecordsRef.current.length
        console.log(
          `📊 OBD Data Context: History fetch finished event - collected ${recordCount} records so far`,
          {
            isFetchingHistoryRef: isFetchingHistoryRef.current,
            isHistoryFetchingRef: isHistoryFetchingRef.current,
            timestamp: new Date().toISOString(),
        ...
        setTimeout(() => {
          if (isFetchingHistoryRef.current) {
            ...
            console.log(
              `📊 OBD Data Context: Final history record count after onObdEldFinish: ${finalRecordCount}`,
              {
            ...
            console.log(
              "⚠️ OBD Data Context: History fetch flag was cleared before onObdEldFinish wait completed",
            )
          }
    ...
    const obdErrorDataListener = JMBluetoothService.addEventListener(
      ...
      (errorData: any) => {
        try {
          ...
          const canErrorCodes = Array.isArray(errorData?.canErrorCodes)
            ? errorData.canErrorCodes
            : []
          ...
          if (obdDtcCodes.length > 0) {
            ...
            if (p0195) {
              ...
              console.log("🔍 P0195 Details:", {
                code: "P0195",
                description:
                  p0195.description || 'Engine Oil Temperature Sensor "A" Circuit Malfunction',
                system: "Powertrain",
                timestamp: new Date().toISOString(),
              })
            }
          ...
          ecuList.forEach((ecu: any, index: number) => {
            ...
            console.log(
              `📋 OBD Data Context: ECU ${index} - ID: ${ecu?.ecuId || ecu?.ecuIdHex || "unknown"}, Codes: ${JSON.stringify(codes)}`,
            )
          })
          ...
          const codesToProcess: Array<{ code: string; type: string; description?: string }> = []
          ...
          const obdDtcCodesList = codesToProcess.filter(
            (c) => c.type === "obd_dtc" || c.type === "unknown",
          )
          ...
          canErrorCodesList.forEach((codeInfo, index) => {
            ...
            console.log(
              `🔧 OBD Data Context: CAN Error: ${decoded.code} - ${decoded.faultDescription}`,
            )
            ...
            malfunctionRecords.forEach((newRecord) => {
              ...
              if (code) {
                ...
                if (codeMap.has(code)) {
                  ...
                  console.log(
                    `🔄 OBD Data Context: Updated existing code ${code} timestamp (grouped)`,
                  )
                } else {
          ...
          const allCodes = uniqueRecordsForBackend.flatMap((record) =>
            record.codes.map((c) => c.code),
          )
          console.log(
  ...
  useEffect(() => {
    ...
    const runLocalSync = async () => {
      ...
      if (dataBufferRef.current.length === 0) {
        ...
        console.log(
          "⏭️  Local API: Debug - isAuthenticated:",
          isAuthenticated,
          "driverProfile:",
          !!driverProfile,
          "isConnected:",
  ...
  const onGpsNoteAdded = useCallback(
    async (note: string) => {
      try {
  ...
  const addDriverNote = useCallback(
